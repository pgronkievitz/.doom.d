...
#+name: fonts
#+begin_src elisp :noweb yes
(setq doom-font (font-spec :family "Agave Nerd Font Mono" :size 22)
      doom-unicode-font (font-spec :family "Agave Nerd Font Mono" :size 22)
      doom-big-font (font-spec :family "Agave Nerd Font Mono" :size 44)
      doom-variable-pitch-font (font-spec :family "Merriweather" :size 22)
      doom-serif-font (font-spec :family "UbuntuMono Nerd Font Mono" :size 22))
#+end_src
...
(add-to-list 'org-modules 'org-habit 'org-secretary)
...
#+end_src
** Misc
For some reason my emacs on work laptop doesn't get that I have ~~/.local/bin~ in my path, so here it comes.
#+name: miscconf
#+begin_src elisp
(when (eq (system-name) "themis")
  (add-to-list exec-path "/home/pg/.local/bin"))
#+end_src
** base :noexport:
